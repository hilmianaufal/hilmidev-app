...
BACKUP_DIR="storage/app/whatsapp-button-backup-$STAMP"

mkdir -p "$BACKUP_DIR/resources/views/layouts"
mkdir -p resources/views/components

if [ -f resources/views/layouts/app.blade.php ]; then
    cp resources/views/layouts/app.blade.php \
        "$BACKUP_DIR/resources/views/layouts/app.blade.php"
fi

if [ -f config/contact.php ]; then
    mkdir -p "$BACKUP_DIR/config"
    cp config/contact.php "$BACKUP_DIR/config/contact.php"
fi
...
WHATSAPP_MESSAGE="Halo HilmiDev, saya ingin berkonsultasi mengenai website, source code, atau membership."
WHATSAPP_LABEL="Chat WhatsApp"
ENV
fi

if [ -f .env.example ] && ! grep -q "^WHATSAPP_NUMBER=" .env.example; then
    cat >> .env.example <<'ENV'
...
# ============================================================
# COMPONENT TOMBOL WHATSAPP
# ============================================================

cat > resources/views/components/whatsapp-button.blade.php <<'BLADE'
@php
...
if (! str_contains($content, '<x-whatsapp-button')) {
    $position = strrpos($content, '</body>');

    if ($position === false) {
        throw new RuntimeException(
            'Tag </body> tidak ditemukan pada layout publik.'
        );
    }
...
echo "Tombol WhatsApp berhasil dipasang ke layout publik.\n";
PHP
...
echo ""
echo "============================================================"
echo "TOMBOL WHATSAPP BERHASIL DIBUAT"
echo "============================================================"
echo "Nomor saat ini : 6281234567890"
echo "Ubah nomor di  : .env"
